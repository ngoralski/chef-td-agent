...
action :create do
  ...
  template "#{node['td_agent']['etc_folder']}/conf.d/#{new_resource.source_name}.conf" do
...
action :delete do
  file "#{node['td_agent']['etc_folder']}/conf.d/#{new_resource.source_name}.conf" do
    action :delete
    only_if { ::File.exist?("#{node['td_agent']['etc_folder']}/conf.d/#{new_resource.source_name}.conf") }
    notifies reload_action, 'service[td-agent]'
  end
